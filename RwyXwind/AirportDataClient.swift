...
class AirportDataClient {
    ...
    /// Deprecated
    /// ICAO --> IATA
    func getIata(fromIcao: String, completionHandler: @escaping (_ iata: String?, _ errorString: String?) -> Void) {
        // Create the session
        let session = URLSession.shared
        var parsedResult: NSDictionary!
        ...
        let task = session.dataTask(with: request as URLRequest, completionHandler: { data, response, downloadError in
            if downloadError == nil {
                ...
                if parsedResult.value(forKey: JSONKeys.status) as? Int !=  Status.success.id {
        ...
        // Run the task until it is completed
        task.resume()
    }
}
